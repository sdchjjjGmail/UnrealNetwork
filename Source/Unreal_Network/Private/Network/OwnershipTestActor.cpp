...
#include "Network/OwnershipTestActor.h"
...
AOwnershipTestActor::AOwnershipTestActor()
{
	...
	OverlapCollision = CreateDefaultSubobject<USphereComponent>(TEXT("SphereComopnent"));
	OverlapCollision->SetupAttachment(RootComponent);
	OverlapCollision->SetSphereRadius(400.f);
...
void AOwnershipTestActor::Tick(float DeltaTime)
{
	...
	if (HasAuthority())
	{
		...
		UGameplayStatics::GetAllActorsOfClass(GetWorld(), ANetworkRoll::StaticClass(), Actors);
		...
		{
			SetOwner(NextOwner);	// 자신에게 가장 가까운 ANetworkRoll을 오너로 처리
